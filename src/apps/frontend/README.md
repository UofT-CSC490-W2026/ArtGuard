...
- **Results (`/results`, protected)** — Shows the latest analysis from `localStorage` (written right after upload; see [Results page: local snapshot](#results-page-local-snapshot)): authenticity score, verdict, RAG-style explanation when present, **patch heatmap** overlay when the analysis includes patch data, and **Download** (opens the browser print dialog for a simple printable view).
