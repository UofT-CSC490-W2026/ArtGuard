...
- **RAG-Powered Explanations** — Each prediction is accompanied by a natural-language explanation generated by Claude 4.5 Sonnet, grounded in a knowledge base of art-historical documents via Amazon Bedrock RAG
...
4. Generates a human-readable explanation via AWS Bedrock RAG (Retrieval-Augmented Generation) using Claude 4.5 Sonnet grounded in art-historical documents from the Metropolitan Museum and Wikidata
...
| **RAG** | AWS Bedrock (Claude 4.5 Sonnet) + OpenSearch Serverless vector store |
...
```bash
export API_BASE="https://d1b5yxlog377uv.cloudfront.net"   # Ask a team member for the URL
